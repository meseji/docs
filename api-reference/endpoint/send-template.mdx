...
<Note>
  Template messages are required to initiate conversations with customers
...
const response = await fetch(
  "https://api.meseji.app/api/v1/whatsapp/messages/template",
  {
    method: "POST",
    headers: {
...
<Tip>
  **Pro tip:** Use the [Get Template
  Variables](/api-reference/endpoint/get-template-variables) endpoint first to
  see what variables your template requires.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Invalid phone number">
    Phone numbers must include country code without `+` or spaces (e.g.,
    `919876543210`).
  </Accordion>
